...
//
//        ofPoint pos = ofPoint(1920/2 - longestWidth*.3, i*40+100);
                ofPoint pos = ofPoint(ofGetWidth()/2, i*40+100);
...
//    ofSetColor(255);
    ...
    if (xmovement < 11) {
        ...
        if(faceLock > 0 && faceLock < 2800*timeScale){
                for(int i=0; i<words.size(); i++){
                    //words[i].alpha = ofMap(faceLock, (i * 60), (i * 60)+60, 0, 1);
                    int lineNum = words[i].line;
                    float target = ofMap(faceLock, (lineNum * 120*timeScale), (lineNum * 120*timeScale)+120*timeScale, 0, 1, true);
    ...
    if (faceLock > 2800*timeScale && faceLock < 3000*timeScale) {
        for(int i=0; i<words.size(); i++){
...
//            }
            ...
            if (faceLock > 2900 && words[i].bPositive == true) {
                words[i].alpha = .98*words[i].alpha;
            }
        }
        if (!tracker.getFound()) {
            faceLock = 0;
    ...
    else if (faceLock >= 3000*timeScale) {
        
        ofSetColor(0,0,0,ofMap(faceLock, 3000*timeScale, 3000*timeScale + 4*60 * timeScale, 0, 255, true));
